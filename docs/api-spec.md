...
> - ❌ **未实现** - 仅为规划，代码未实现
>
> **运行时口径（2025-12-30）**：后端已切换为纯 Chroma 向量库；整体归并候选 Top-K=3、时间窗 180 天；新建 Topic 默认按热度保留比例 `GLOBAL_MERGE_NEW_TOPIC_KEEP_RATIO`（当前 0.5）。API 字段未变更，语义保持兼容。
...
- ✅ POST `/ingest/run`
  ...
  - 说明：每日3次归并，每次2阶段：
    - **上午（AM）**：12:05阶段一，12:20阶段二（处理8:00/10:00/12:00采集数据）
...
- **触发时机**：每次新事件归并时执行（12:05 / 18:05 / 22:05，Asia/Shanghai）
- **归一化步骤**：
...
### 半日归并（Halfday Merge）
- **上午（AM）**：8:00、10:00、12:00 三次采集，12:05 触发归并
...
### 整体归并（Global Merge）
- **触发时机**：12:20 / 18:20 / 22:20（新事件归并后约 15 分钟）
