...
> **运行口径快照（2026-01-22）**
> - 向量库：已统一为 **Chroma**（不再使用 pgvector）。  
> - 整体归并：候选时间窗 180 天，Top-K=3；新建/合并时同步写占位摘要向量。  
> - 新建 Topic 热度截断：`GLOBAL_MERGE_NEW_TOPIC_KEEP_RATIO` 当前 1.0（不截断），全部新建 Topic 保留。  
> - 采集侧新增噪音过滤（如“点击查看更多实时热点”及热榜列表页链接）。
> - 阶段1/2 逻辑已落地，详见 merge-logic.md。
...
- 归并逻辑：
  ...
  - **归并频率**：每日 4 次归并（MORN/AM/PM/EVE）：
    - MORN：08:00 采集 → 08:05 阶段一 → 08:20 阶段二
    - AM：10:00/12:00 采集 → 12:05 阶段一 → 12:20 阶段二
    - PM：14:00/16:00/18:00 采集 → 18:05 阶段一 → 18:20 阶段二
    - EVE：20:00/22:00 采集 → 22:05 阶段一 → 22:20 阶段二
  - **半日归并**：针对半日内所有采集批次的事件，判断某事件及其关联事件在半日内出现 ≥2 次则归并保留，仅出现 1 次的事件直接丢弃（不入库）。
    - MORN：08:00 单次采集
    - AM：10:00、12:00 两次采集
    - PM：14:00、16:00、18:00 三次采集
    - EVE：20:00、22:00 两次采集
...
- LLM 接入：支持自定义与云端（OpenAI/Azure/Qwen 等），批量判定、响应截断、函数式输出。
  ...
  - **相似度判定方案**：使用 Chroma 向量数据库（持久化 `echoman_embeddings`）进行快速检索，召回 Top-K=3 候选后再用 LLM 做精确判定；候选时间窗 180 天。
  - **向量数据库选择**：**已落地为纯 Chroma**（高维友好）；pgvector 路线已弃用。
...
- 🚧 **阶段 2：AI 处理与 AI 对话**（进行中30%）
  - **实现状态**：
    ...
    - ✅ 数据模型设计（`topic_period_heat`, `llm_judgements`, `summaries`等）
...
- 调度与任务队列：Celery + Redis，Beat 定时（8:00-22:00 每 2 小时，共 8 次）触发采集；每日 4 次归并（08:05/12:05/18:05/22:05 阶段一，08:20/12:20/18:20/22:20 阶段二）。
- 数据层：
...
- 向量检索策略（现网口径）：
  - **Chroma 作为唯一向量库**：事件向量（source_item）+ 摘要向量（topic_summary）都存于 Chroma；整体归并召回基于摘要向量 Top-K=3，时间窗 180 天。
...
- source_items（采集原子项）
  ...
  - **period_merge_group_id**（归并组 ID，同组事件会被合并）
  - **period**（归并时段标识：MORN/AM/PM/EVE，如 "2026-01-22_MORN"）
  - **occurrence_count**（归并周期内出现次数，≥2 才保留）
  - embedding_id（可空）
...
- topics（主题/事件）
  ...
  - **current_heat_normalized**（归一化热度峰值，跨归并周期取最大）
  - summary_id（当前主题摘要快照）
...
- **topic_period_heat**（主题归并周期热度记录）
  - id, topic_id, date（日期）, period（时段：MORN/AM/PM/EVE）
  - **heat_normalized**（周期内归一化热度，0-1）
  - **heat_percentage**（占归并周期总热度百分比）
  - source_count（归并周期内的 source_item 数量）
  - created_at, updated_at
  - 注：每次归并完成后更新此表，用于前端热度趋势图（每日 4 个数据点）
...
- 暂存策略：每次采集的数据先暂存到 Redis 或临时表（标记为 `pending_event_merge`，带时段标识 MORN/AM/PM/EVE），等待对应时段归并触发。
  - MORN：08:00 采集 → 08:05 触发归并
  - AM：10:00、12:00 采集 → 12:05 触发归并
  - PM：14:00、16:00、18:00 采集 → 18:05 触发归并
  - EVE：20:00、22:00 采集 → 22:05 触发归并
...
- **触发时机**：每天 4 次（08:05/12:05/18:05/22:05，Asia/Shanghai），采集与热度归一化后执行。
- **输入**：归并周期内所有采集批次暂存的 source_items。
  - MORN：08:00 采集（约 20-30 条）
  - AM：10:00、12:00 采集（约 40-60 条）
  - PM：14:00、16:00、18:00 采集（约 60-90 条）
  - EVE：20:00、22:00 采集（约 40-60 条）
- **流程**：
  1. **向量检索 + 标题聚类**：
  ...
  2. **LLM 精确判定**（Qwen3-32B）：
     ...
     - Prompt 示例：
       ...
       ```
     - 将同组事件标记相同的 `period_merge_group_id`。
  3. **出现次数统计**：
     - 统计每个 `period_merge_group_id` 的 `occurrence_count`（出现次数）。
...
5) **整体归并（Global Merge）**
- **触发时机**：半日归并完成后执行（每日 4 次：08:20/12:20/18:20/22:20，Asia/Shanghai）。
...
- **流程**：
  ...
  4. **热度更新**：
     - 更新 `topics.current_heat_normalized`（取历史峰值）。
     - 在 `topic_period_heat` 表中插入或更新记录：
       - `topic_id`, `date`, `period`（MORN/AM/PM/EVE）, `heat_normalized`, `heat_percentage`, `source_count`。
  5. **指标计算与前端更新**：
...
9) 自由模式（Agent）
- 入口：`POST /chat/ask`，`mode=global`，需携带 `free_token`（由邀请码校验接口发放）。
- 访问控制：
  - `free_mode_invites`（邀请码表，含有效期与使用次数）
  - `free_mode_access_tokens`（访问令牌表，默认 7 天）
- Agent 规划：
  - 先判断意图（闲聊 / 检索 / 查询 / 分析），仅在需要时调用工具。
  - 支持对话历史（最近 8 条）参与意图判断与追问补充条件。
- 工具：
  - `topic_search`：关键词检索 + 向量检索并合并；向量相似度阈值 0.7，少于 3 条则按实际返回。
  - `topic_query`：基于 `topic_item_mv` 物化视图进行字段化查询（含摘要、时间、回声长度/强度、代表 item）。
- 输出约束：
  - 默认输出 10 条（不足则按实际条数）
  - 每条必须包含回声长度与回声强度
  - 总结末尾必须给出“本次命中 X 条话题...”提示
...
- **新增：半日热度追踪**：
  - 每个 topic 在 `topic_period_heat` 表中记录每半日的归一化热度和占比（每日 2 个数据点）。
  - 前端可基于此绘制热度趋势图，观察事件的生命周期热度变化（粒度更细，可看到半日内热度波动）。
  - 热度峰值识别：`SELECT MAX(heat_normalized) FROM topic_period_heat WHERE topic_id = ?`。
...
- 关键变量：
  ...
  - 运行时归并触发时间：08:05/12:05/18:05/22:05（阶段一），08:20/12:20/18:20/22:20（阶段二）
  - 兼容配置（历史口径）：`CRON_HALFDAY_MERGE_SCHEDULE_NOON` / `CRON_HALFDAY_MERGE_SCHEDULE_NIGHT`，`CRON_GLOBAL_MERGE_SCHEDULE_NOON` / `CRON_GLOBAL_MERGE_SCHEDULE_NIGHT`
...
- 图阶段建议（两层归并流水线，半日执行一次）：
  ...
  **第一阶段：采集与暂存**
  ...
  - store_pending：暂存到 Redis 或临时表，标记 `pending_event_merge` + 时段标识（MORN/AM/PM/EVE）。
  
  **第二阶段：新事件归并（08:05 / 12:05 / 18:05 / 22:05 触发，每日 4 次）**
  - heat_normalization：对半日内所有暂存数据进行热度归一化（Min-Max + 平台权重）。
  - halfday_vector_cluster：使用 Qwen3-Embedding-8B 向量化 + 标题聚类，形成候选归并组。
  - halfday_merge_judge_llm：批量调用 Qwen3-32B 判定同组事件，标记 `period_merge_group_id`。
  - filter_by_occurrence：统计 `occurrence_count`，保留 ≥2 次的事件，丢弃单次出现。
  - aggregate_halfday_heat：计算每个归并组的半日热度（平均或最大）。
  
  **第三阶段：整体归并（08:20 / 12:20 / 18:20 / 22:20 触发，每日 4 次）**
  ...
  - update_topic_heat：更新 `topics.current_heat_normalized`（峰值）和 `topic_period_heat` 表（记录半日热度）。
  ...
  - notify_frontend：通过 WebSocket 或轮询通知前端更新（每日 4 次实时更新）。
...
> 说明：当前运行实际包含 **MORN/AM/PM/EVE** 四个归并时段（08:05/12:05/18:05/22:05）。以下时间线为历史 AM/PM 口径示例，仅用于理解流程结构。
...
12:20 → 上午整体归并触发
  ...
  │   └─ new → 创建新 topic
  ├─ 步骤4: 热度更新（topic_period_heat 表，period=AM）
...
18:20 → 下午整体归并触发
  ...
  │   ├─ merge → 追加 topic_node，更新 last_active
  │   └─ new → 创建新 topic
  ├─ 步骤4: 热度更新（topic_period_heat 表，period=PM）
  ├─ 步骤5: 分类与摘要（LLM 分类 + 增量摘要）
  ├─ 步骤6: 向量写入（纯 Chroma 存储）
  ├─ 步骤7: 指标聚合（category_day_metrics）
  └─ 步骤8: 通知前端（轮询，第 2 次更新）
...
```
source_items (采集原子项)
  ↓ [暂存标记: pending_event_merge + period (MORN/AM/PM/EVE)]
  ↓
  ↓ [08:05 清晨归并 / 12:05 上午归并 / 18:05 下午归并 / 22:05 傍晚归并]
  ↓ - 热度归一化 → heat_normalized
  ↓ - 向量聚类 + LLM 判定 → period_merge_group_id
  ↓ - 出现次数统计 → occurrence_count
  ↓ - 筛选保留 (>= 2次)
  ↓
source_items (保留事件, 标记: pending_global_merge)
  ↓
  ↓ [08:20 清晨整体归并 / 12:20 上午整体归并 / 18:20 下午整体归并 / 22:20 傍晚整体归并]
  ↓ - 向量检索 active topics
  ↓ - LLM 关联判定
  ↓
  ├─→ [merge] topic_nodes (追加时间线)
  │              ↓
  │         topics (更新 last_active, current_heat_normalized 峰值)
  │              ↓
  │         topic_period_heat (记录周期热度, period=MORN/AM/PM/EVE)
  │
  └─→ [new] topics (创建新事件)
                ↓
           topic_nodes (首个节点)
                ↓
           topic_period_heat (首周期热度, period=MORN/AM/PM/EVE)
```

**每日热度追踪示例：**
```
topic_period_heat 表记录
...
4. **监控指标**：
   - 半日归并（每日 4 次）：处理耗时、保留率、丢弃率
   - 整体归并（每日 4 次）：merge 率、new 率、候选召回数
   - 热度归一化：各平台热度分布、异常值检测、上下半日对比
   - LLM 调用：Token 用量、延迟、失败率、半日累计消耗
   - 前端更新：WebSocket 推送延迟、更新成功率（每日 4 次）
...
- ❌ 半日归并任务（12:15, 22:15）已废弃（当前为 08:05/12:05/18:05/22:05）
- ❌ 整体归并任务（12:30, 22:30）已废弃（当前为 08:20/12:20/18:20/22:20）
- ❌ 分类指标计算任务
