...
### 每日任务类型
1. **采集任务**：8次/天（每2小时）
2. **归并任务**：8次/天（4个周期×2阶段：MORN/AM/PM/EVE）
3. **统计任务**：1次/天（凌晨）
...
| 时间 | 阶段 | 任务 | 说明 |
|------|------|------|------|
| **08:05** | 阶段一 | 新事件归并 | 处理 8:00 采集数据 |
| **08:20** | 阶段二 | 全局归并 | 关联演进，生成/更新主题 |

**Crontab 配置**：
```python
# 阶段一
"halfday-merge-morn": crontab(hour=8, minute=5)
# 阶段二
"global-merge-morn": crontab(hour=8, minute=20)
```
...
| 时间 | 阶段 | 任务 | 说明 |
|------|------|------|------|
| **12:05** | 阶段一 | 新事件归并 | 处理 8:00/10:00/12:00 采集数据 |
| **12:20** | 阶段二 | 全局归并 | 关联演进，生成/更新主题 |

**处理数据**：AM 时段（10:00、12:00）的2次采集结果

**Crontab 配置**：
```python
# 阶段一
"halfday-merge-am": crontab(hour=12, minute=5)
# 阶段二
"global-merge-am": crontab(hour=12, minute=20)
```
...
```python
# 阶段一
"halfday-merge-pm": crontab(hour=18, minute=5)
# 阶段二
"global-merge-pm": crontab(hour=18, minute=20)
```
...
```python
# 阶段一
"halfday-merge-eve": crontab(hour=22, minute=5)
# 阶段二
"global-merge-eve": crontab(hour=22, minute=20)
```
...
10:00 📥 【采集 #2】    │ AM 周期（3次采集）
      │                 │
12:00 📥 【采集 #3】────┘
12:05 🔄 【上午归并 - 阶段一】新事件归并（去噪过滤）
12:20 🔄 【上午归并 - 阶段二】全局归并（关联演进）
      │
14:00 📥 【采集 #4】────┐
      │                 │
16:00 📥 【采集 #5】    │ PM 周期（3次采集）
      │                 │
18:00 📥 【采集 #6】────┘
18:05 🔄 【下午归并 - 阶段一】新事件归并（去噪过滤）
18:20 🔄 【下午归并 - 阶段二】全局归并（关联演进）
      │
20:00 📥 【采集 #7】────┐
      │                 │ EVE 周期（2次采集）
22:00 📥 【采集 #8】────┘
22:05 🔄 【傍晚归并 - 阶段一】新事件归并（去噪过滤）
22:20 🔄 【傍晚归并 - 阶段二】全局归并（关联演进）
      │
23:59 ─────────────────────────────────────────────────
```
...
**时间间隔**：15分钟
- 阶段一：08:05/12:05/18:05/22:05
- 阶段二：08:20/12:20/18:20/22:20
- 确保阶段一完成后再执行阶段二
