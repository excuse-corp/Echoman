...
【归并总体流程】
每日执行4轮归并（早间 08:05-08:20，上午 12:05-12:20，下午 18:05-18:20，傍晚 22:05-22:20）：
  ┌─────────────────────────────────────────────────────────────┐
  │ 阶段一：新事件归并（本模块，08:05/12:05/18:05/22:05）       │
  │ - 对新爬取数据去噪                                           │
  │ - 热度归一化（Min-Max + 平台权重）                          │
  ...
  └─────────────────────────────────────────────────────────────┘
                             ↓
  ┌─────────────────────────────────────────────────────────────┐
  │ 阶段二：整体归并（global_merge.py，08:20/12:20/18:20/22:20）│
  │ - 与历史Topic库比对                                          │
  │ - 决策：归入已有主题 or 创建新主题                          │
  │ - 输出：更新Topics表 + 前端数据更新                         │
  └─────────────────────────────────────────────────────────────┘
...
class EventMergeService:
    ...
    职责：
    - 对归并周期（MORN/AM/PM/EVE）内新采集的数据去噪、验证真实热点
    - 过滤单次出现的噪音数据（出现次数 < 2）
    - 保留经过验证的真实热点事件
    ...
    async def run_event_merge(self, period: str) -> Dict[str, Any]:
        ...
        Args:
            period: 归并周期标识（如 "2025-10-29_MORN/AM/PM/EVE"）
